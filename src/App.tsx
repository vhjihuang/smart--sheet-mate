  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto p-6 flex flex-col gap-6 bg-gray-50 min-h-screen font-sans antialiased">
      ...
      <DndContext onDragStart={handleDragStart} onDragEnd={handleDragEnd} collisionDetection={customCollisionDetection}>
        ...
        {/* Main Layout Container - Conditional Layout Switch */}
        {!isConfigCollapsed ? (
          /* Vertical Layout (Expanded): Config on top, Workspace below */
          <div className="flex flex-col gap-4 flex-1 overflow-hidden">
            {/* Top: Box1 (Source) + Box2 (Target) side by side */}
            <div className="flex flex-row gap-4">
              {/* Box 1: Source File Configuration */}
              <section className="flex-1 bg-white border border-gray-200 rounded-xl p-5 shadow-sm min-w-0">
              ...
              {/* Box 2: Target Template Configuration */}
              <section className="flex-1 bg-white border border-gray-200 rounded-xl p-5 shadow-sm min-w-0">
                <div className="flex items-center gap-3 flex-wrap pb-4 border-b border-gray-100">
            ...
            {/* Bottom: Aside (Source Fields) + Mapping Center side by side */}
            <div className="flex flex-row gap-6 flex-1 overflow-hidden">
              ...
              {/* Mapping Area */}
              <section className="flex-1 bg-white border border-gray-200 rounded-xl p-4 shadow-sm overflow-hidden flex flex-col gap-3 mb-10 min-w-0">
          ...
          /* Horizontal Layout (Collapsed): Original sidebar + workspace layout */
          <div className="flex flex-row gap-6 items-start flex-1 overflow-hidden animate-in fade-in slide-in-from-top-2">
            {/* 左侧边栏: 源字段 */}
            <aside className="w-64 flex-shrink-0 sticky top-0 max-h-[calc(100vh-8rem)] overflow-y-auto bg-white border border-gray-200 rounded-xl p-4 shadow-sm flex flex-col gap-4">
              <div className="flex items-center gap-2 border-b border-gray-100 pb-3">
                <Layers size={16} className="text-blue-500" />
      ...
      {/* Floating Footer */}
      <footer className="mt-auto py-3 border-t border-gray-200 flex justify-end sticky bottom-0 bg-gray-50/90 backdrop-blur-sm z-50">
        <Button onClick={handleDownload} className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 h-auto text-xs font-semibold shadow-md transition-all hover:scale-[1.02] active:scale-95 rounded-lg">
